...
import { useGenerateAvatarUploadUrl, useSetAvatarPhoto, useSetAvatarPreset, uploadAvatarPhoto } from '@/state/avatar-client';
...
function AvatarPickerContent() {
  ...
  const pickPhoto = async () => {
    ...
    setUploading(true);
    try {
      await uploadAvatarPhoto(result.assets[0].uri, () => generateAvatarUploadUrl({}), setAvatarPhoto);
    } catch {
  ...
  return (
    <Screen>
      ...
      <Card style={styles.photoCard}>
        ...
        {uploading ? <ActivityIndicator color={colors.brand} /> : <PrimaryButton icon="upload" label="Nahrát" onPress={() => void pickPhoto()} tone="surface" />}
      </Card>
        ...
        {AVATAR_PRESETS.map((preset) => {
          ...
          return (
            <Pressable
              ...
              key={preset.id}
              onPress={() => void setAvatarPreset({ avatarId: preset.id })}
