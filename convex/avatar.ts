...
import { canChangeAvatar, VIP_YEARLY_AVATAR_CHANGES } from './avatarRules';
...
async function checkAvatarChangeAllowed(ctx: any, userId: any) {
  ...
  if (!canChangeAvatar(vip, timestamps, Date.now())) {
...
/** Mirrors handle.ts's getMyHandleChangeStatus — drives the avatar picker's remaining-changes/disabled state without duplicating avatarRules.ts's limit logic client-side. */
export const getMyAvatarChangeStatus = query({
  ...
  handler: async (ctx: any) => {
    ...
    const changesUsedInWindow = vip
      ? timestamps.filter((timestamp: number) => now - timestamp < 365 * 24 * 60 * 60 * 1000).length
      : timestamps.length;
    return { isGuest: false, changesUsedInWindow, changesAllowed: vip ? VIP_YEARLY_AVATAR_CHANGES : 1 };
  },
});
