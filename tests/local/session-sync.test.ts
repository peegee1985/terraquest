...
describe('processDueSyncEvents', () => {
  it('on confirmation: marks the event sent, completes the session, and prunes only points up to endedAt', async () => {
    ...
    const result = await processDueSyncEvents({ outbox, trackPoints, session, transport }, endedAt + 1);
  ...
  it('never deletes points from a newer session that has since reused the slot, and never stomps its active status', async () => {
    ...
    const transport = async (): Promise<SyncResult> => ({ ok: true });
    await processDueSyncEvents({ outbox, trackPoints, session, transport }, oldEndedAt + 1);
  ...
  it('on failure: schedules a backoff retry without touching points or session status', async () => {
    ...
    const now = endedAt + 1;
    const result = await processDueSyncEvents({ outbox, trackPoints, session, transport }, now, {
      baseDelayMs: 1000,
      maxDelayMs: 60_000,
      maxAttempts: 5,
    });
  ...
  it('gives up after exceeding the retry budget, leaving the event out of listDue for good', async () => {
    ...
    await processDueSyncEvents({ outbox, trackPoints, session, transport }, now, retryOptions); // attempt 1: normal retry
    now += 10_000;
    await processDueSyncEvents({ outbox, trackPoints, session, transport }, now, retryOptions); // attempt 2: gives up
